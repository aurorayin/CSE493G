...
class FullyConnectedNetImproved(object):
    ...
    def loss(self, X, y=None):
        ...
        layer_input = X
        caches = [None for i in range(self.num_layers)]
        dropout_caches = [None for i in range(self.num_layers - 1)]
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization == "batchnorm":
                ...
                layer_input, caches[i] = affine_bn_relu_forward(layer_input, W, b, gamma, beta, self.bn_params[i])
            else:
                layer_input, caches[i] = affine_relu_forward(layer_input, W, b)
            if self.use_dropout:
                layer_input, dropout_caches[i] = dropout_forward(layer_input, self.dropout_param)
    
        # forward pass for the last layer
        W = self.params["W" + str(self.num_layers)]
        b = self.params["b" + str(self.num_layers)]
        scores, caches[-1] = affine_forward(layer_input, W, b)
        ...
        for i  in range(self.num_layers - 1, 0, -1):
            ...
            if self.normalization == "batchnorm":
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, caches[i - 1])
